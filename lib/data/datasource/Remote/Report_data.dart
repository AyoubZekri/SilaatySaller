...
import 'package:Saller/core/class/Sqldb.dart';
import 'package:Saller/core/class/SyncServer.dart';
...
class ReportData {
  ...
  Future<Map<String, Object?>> deleteReport(Map<String, Object?> data) async {
    final uuid = data["uuid"] as String;
    try {
      final result = await db.delete("reports", "uuid = ?", [uuid]);
  ...
  Future<Map<String, Object?>> ShwoReport() async {
    try {
      final result =
          await db.readData("SELECT * FROM reports Where report_id = ?", [id]);
      return {
        "status": 1,
        "data": {"Report": result}
      };
